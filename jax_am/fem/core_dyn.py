...
config.update("jax_enable_x64", True)
 # add this for memory pre-allocation
...
class FEM:
    ...
    def __post_init__(self):
        self.points = self.mesh.points #(num_nodes, dim): nodal coord for each node
        ...
        print('num of quads is', self.num_quads)
        ...
        self.mass_BCOO = self.get_mass_matrix()
        ...
        self.v_grads_JxW = self.shape_grads[:, :, :, None, :] * self.JxW[:, :, None, None, None] #B * JxW
    ...
    def get_shape_grads(self):
        ...
        shape_grads_physical = (self.shape_grads_ref[None, :, :, None, :] @ jacobian_deta_dx)[:, :, :, 0, :] #BiI
        JxW = jacobian_det * self.quad_weights[None, :] # (num_cells, num_quads)
        return shape_grads_physical, JxW
    
    def get_mass_matrix(self):
        # def Mn(N, JxW):
        #     #N_til：（num_quads, num_nodes)
        #     #JxW: (num_quads)
        #     mass = self.density * np.sum(N[:, :, None] * N[:, None, :] * JxW[:, None, None], axis = 0) #(num_cells, num_nodes, num_nodes)
        #     I = np.eye(self.dim)
        #     M =   I[None, :, None, :] * mass[:, None, :, None] #(num_cells, num_nodes, vec, num_nodes, vec)
        #     return M
        ...
        def Mn(N, JxW):
            ...
            #JxW: (num_quads)
            #jax.debug.print("N: {}", N)
            mass = self.density * np.sum(N[:, :, None] * N[:, None, :] * JxW[:, None, None], axis = 0) #(num_nodes, num_nodes)
            #jax.debug.print("mass: {}", mass)
            I = np.eye(self.dim)
            M =   I[None, :, None, :] * mass[:, None, :, None] #(num_nodes, vec, num_nodes, vec)
            return M       
        vmap_Mn = jax.jit(jax.vmap(Mn))
        num_cuts = 20 #batch input
        if num_cuts > len(self.cells): #len(self.cells): num of elements
            num_cuts = len(self.cells)
        batch_size = len(self.cells) // num_cuts
        N_cells = np.repeat(self.shape_vals[None, :, :], self.num_cells, axis = 0)
        input_collection = [N_cells, self.JxW]
        values = []
        for i in range(num_cuts): #loop over each element
            ...
            mass = vmap_Mn(*input_col)
            values.append(mass) #append uiI (num_ele, num_nodes, vec, num_nodes, vec)
        
        gpu = jax.devices("gpu")[0]
        mass_jacs = np.vstack(jax.device_put(values, gpu))        
        V = mass_jacs.reshape(-1)
        inds = (self.vec * self.cells[:, :, None] + onp.arange(self.vec)[None, None, :]).reshape(len(self.cells), -1) #what's this?
        ...
        #self.mass_scipy = scipy.sparse.csr_array((V, (I, J)), shape=(self.num_total_dofs, self.num_total_dofs))
        
        #lumped mass matrix
        mass_lumped = scipy.sparse.csr_array((V, (I, J)), shape=(self.num_total_dofs, self.num_total_dofs)).sum(axis = 1)
        self.mass_scipy = scipy.sparse.diags(mass_lumped)
        mass_BCOO = BCOO.from_scipy_sparse(self.mass_scipy).sort_indices()
    ...
    def compute_body_force_by_fn(self):
        ...
        if self.source_info is not None:
            body_force_fn = self.source_info #how to add the body force term
    ...
    def get_laplace_kernel(self, tensor_map): #heterogenous materials
        def laplace_kernel(cell_sol, cell_shape_grads, cell_v_grads_JxW, *cell_internal_vars): #*cell_internal_vars is the parameters
            # (1, num_nodes, vec, 1) * (num_quads, num_nodes, 1, dim) -> (num_quads, num_nodes, vec, dim) #vec refers to the dimension of the variable, for heat vec = 1, for disp, vec = 3
            ...
            ###############################################
            #to be changed in material calibration
            u_physics = jax.vmap(tensor_map)(u_grads_reshape, *cell_internal_vars).reshape(u_grads.shape)  #stress \sigma_ij in solid mechanics  what does * mean here?
            # (num_quads, 1, vec, dim) * (num_quads, num_nodes, 1, dim) -> (num_quads,num_nodes, vec, dim)  -> sum on 0, -1 ->(num_nodes, vec) (num_nodes, vec)
            val = np.sum(u_physics[:, None, :, :] * cell_v_grads_JxW, axis=(0, -1)) #internal force fiI [num_nodes, vec]
            return val #[num_nodes, vec]
        return laplace_kernel  #[num_nodes, vec]
    ...
    def split_and_compute_cell(self, cells_sol, np_version, jac_flag, **internal_vars): #what's this one? core of automatic differentiation
        '''
        returns:
        ...
        '''
        def value_and_jacrev(f, x): #f here refers to the function fiI=f(ujJ)
            y, pullback = jax.vjp(f, x) #y:fiI
            basis = np.eye(len(y.reshape(-1)), dtype=y.dtype).reshape(-1, *y.shape) #identity matrix what's this basis?
            jac, = jax.vmap(pullback)(basis) #how to understand this?Ans: here basis is just the unit cotangent vector, i think here is to try to extract every single term in the tangent stiffness matrix
            return y, jac.reshape(self.num_nodes, self.vec, self.num_nodes, self.vec) #why the shape is like this? Ans: this is essentially the dimension of the tangent stiffness matrix

        def value_and_jacfwd(f, x): ##compute \partial f_iI \partial ujI
        ...
        def get_kernel_fn_cell():
            def kernel(cell_sol, cell_shape_grads, cell_JxW, cell_v_grads_JxW, cell_mass_internal_vars, cell_laplace_internal_vars): #returns f_iI
                ...
                if hasattr(self, 'get_tensor_map'):
                    laplace_kernel = self.get_laplace_kernel(self.get_tensor_map()) #get tensor map will return the pk stress
                    laplace_val = laplace_kernel(cell_sol, cell_shape_grads, cell_v_grads_JxW, *cell_laplace_internal_vars)
                else:
                    laplace_val = 0. #(num_nodes, dim)
                
                return laplace_val + mass_val #returns f_iI in one element

            def kernel_jac(cell_sol, *args):
                kernel_partial = lambda cell_sol: kernel(cell_sol, *args) # cell_sol: nodal uiI values; \partial f_iI \partial ujI: tangent stiffness matrix
                return value_and_jacfwd(kernel_partial, cell_sol) # kernel(cell_sol, *args), jax.jacfwd(kernel)(cell_sol, *args) #what derivative is it solving? Answer: tangent stiffness matrix; return fiI and jacobian of fiI

            return kernel, kernel_jac

        kernel, kernel_jac = get_kernel_fn_cell() #it's a function
        fn = kernel_jac if jac_flag else kernel
        vmap_fn = jax.jit(jax.vmap(fn)) #tangent stiffness function
        ...
        # print('Grad_N_til_blocks shape is', self.shape_grads.shape)
        # print('Grad_N_til_blocks type is', type(self.shape_grads))
        # print('self.JxW_blocks shape is', self.JxW.shape)
        # print('self.v_grads_JxW_blocks shape is', self.v_grads_JxW.shape)
        if jac_flag:
            values = []
            jacs = []
            for i in range(num_cuts): #loop over each element
            ...
            jacs = np_version.vstack(jacs) #stack of tangent stiffness \partial f_iI \partial ujJ for different elements [num_ele, num_nodes, vec, num_nodes, vec]
            return values, jacs #what's values and jocobians? return of this split and compute function value (8,8,3) local stiffness matrix? jacs 4608
        else:
            values = []
    ...
    def updatenewton_acc_vel(self, sol): #here I made a serious mistake that took me several days to debug; previously I just used self.sol, which is not updated during newton iteration
        #sol: d^n; sol_v: v^n; sol_a: a^n
        self.sol_a = 1./ (self.beta * self.dt ** 2) * (sol - self.sol_til) #a^n+1
    ...
    def compute_residual_vars_helper(self, sol, weak_form, **internal_vars):
        ...
        weak_form = weak_form.reshape(-1, self.vec) # (num_cells*num_nodes_per_ele, vec) value internal force term
        res = res.at[self.cells.reshape(-1)].add(weak_form) #boundary terms #this part is important, I guess this is trying to assembly the weak form matrix (#internal force matrix) (num_total_nodes*vec) here vec refers to dimension of the problem
        ...
        self.sol_a = self.updatenewton_acc_vel(sol)
        res = res - self.body_force - self.neumann + (self.mass_BCOO @ self.sol_a.reshape(self.num_total_dofs,)).reshape(self.num_total_nodes, self.vec)
    ...
    def compute_residual_vars(self, sol, **internal_vars): #return the residual force:values
        ...
        weak_form = self.split_and_compute_cell(cells_sol, np, False, **internal_vars) # (num_cells, num_nodes, vec)  #def split_and_compute_cell(self, cells_sol, np_version, jac_flag, **internal_vars); weak form:fiI(cell_sol, internal_variable)
    ...
    def compute_newton_vars(self, sol, **internal_vars):
        ...
        V = cells_jac.reshape(-1) #reshape tangent stiffness into a column vector #preparing recast tangent stiffness matrix into a sparse matrix
        #self.cells: nodes ID for each ele [num_ele, num_nodes]
        ...
        self.stiff_scipy = scipy.sparse.csr_array((self.V, (self.I, self.J)), shape=(self.num_total_dofs, self.num_total_dofs))
        A_sp = BCOO.from_scipy_sparse(self.stiff_scipy).sort_indices() +  1./ (self.beta * self.dt**2) *  self.mass_BCOO
        self.A_sp_scipy = self.stiff_scipy + 1./ (self.beta * self.dt**2) * self.mass_scipy
        return self.compute_residual_vars_helper(sol, weak_form, **internal_vars), A_sp
